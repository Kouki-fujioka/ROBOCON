...
float turnTo(float theta_r) {
  ...
  heading_G = atan2(my, mx) * 180 / M_PI;  // 現在の角度を弧度法で求める.それを度数法へ変換
  if (heading_G < 0) heading_G += 360;     // 0-360°の間に収める
  float e = theta_r - heading_G;           // 目標の角度と現在角度との差を求める
  if (e < -180) e += 360;
  if (e > 180) e -= 360;
...
// 試作5
float current_G() {                        // ロボットの現在の角度を0-360°の範囲で求める
  heading_G = atan2(my, mx) * 180 / M_PI;  // 現在の角度を弧度法で求める.それを度数法へ変換
  if (heading_G < 0) heading_G += 360;     // 0-360°の間に収める
  return heading_G;
}
